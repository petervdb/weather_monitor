...
if [ -e $logfile ]; then
  ...
  echo "OK: Temperature: ${temperature} Humidity: ${humidity} Pressure: ${pressure} Altitude: ${altitude} UV_index: ${uv_index} Lux: ${visible} IR: ${ir}|Temperature=${temperature};Humidity=${humidity};Pressure=${pressure};Altitude=${altitude};UV_index=${uv_index};Lux=${visible};IR=${ir}"
else
